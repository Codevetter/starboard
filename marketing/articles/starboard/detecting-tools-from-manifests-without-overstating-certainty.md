...
slug: "detecting-tools-from-manifests-without-overstating-certainty"
target_query: "detect tools from github repository manifests"
search_intent: "Informational - Understanding how repository scanning algorithms can detect tools without false positives"
meta_title: "Detecting tools from manifests without overstating certainty | Starboard"
meta_description: "Learn how Starboard detects tools across repositories by using manifests and SBOMs while clearly defining confidence levels to avoid overstating certainty."
---

## Outline
- **Introduction**: The challenge of mapping what a codebase is actually using versus what it claims to use.
- **Evidence Over Inference**: Why Starboard relies heavily on files that matter: manifests, lockfiles, and Software Bill of Materials (SBOMs).
- **The Confidence Spectrum**: Not all signals are created equal. How confidence scoring works in practice.
- **Handling Ambiguity and Ecosystem Quirks**: The limitations and caveats, particularly for C/C++ or monorepos.
- **Transparency in the UI**: How the Tool Intelligence guide keeps users informed and sets realistic expectations.
- **Internal Links Suggestions**: Proposed links to other Starboard pages.
- **Next Action**: Dive into the code or test the public discovery.
- **Source Notes**: A final, non-publishable section detailing the authoritative codebase sources backing up the claims made.
...
To enhance SEO and guide users through Starboard's capabilities, consider linking to the following pages naturally within the content:
- **Discover Page**: Link to the public Discover feed (`/discover`) when discussing how users can explore popular repositories.
- **Projects**: Link to the Projects dashboard (`/projects`) when mentioning dependency analysis for users' own repositories.
- **Tool Intelligence**: Link directly to the Tool catalog (`/tools`) when explaining how tool detections are aggregated.
...
The claims in this article are derived directly from the current codebase state of the Starboard repository:

1. **Manifest and SBOM scanning:** `src/lib/repo-tools.ts` implements the detection logic. The function `detectToolsFromSbomPackageNames` assigns a confidence of 98 to tools found via `github-sbom`. The function `detectToolsFromManifest` parses paths and provides direct confidence scores (e.g., 90 for `.tf` files, 95 for `cloudflare-workers`).
2. **Confidence Scoring:** `src/lib/repo-tools.ts` defines the hierarchy. Explicit SBOM/manifest hits score in the 90s. `detectToolsFromRepoSignals` assigns a confidence of 55 for a `github-topic`. Inferred text sources are scored lower: `description` (35), `cached-readme` (40), and `ai-metadata` (45).
3. **Merging Detections:** The `mergeToolDetections` function in `src/lib/repo-tools.ts` explicitly keeps the highest confidence score (`existing.confidence = Math.max(existing.confidence, item.confidence);`) and concatenates the sources.
4. **Accuracy Disclaimer and Caveats:** The exact disclaimer text is exported as `TOOL_ACCURACY_DISCLAIMER` in `src/lib/repo-tools.ts` and is presented to the user via `src/components/tool-intelligence-guide.tsx`. This includes the specific notes on C/C++, monorepos, and the fact that language is metadata.
5. **Tool Intelligence Overview:** `public/tools.md` notes that "Source and confidence labels distinguish manifest or SBOM evidence from weaker README, topic, or metadata inference."
6. **File limitations**: The `fetchManifestFiles` function in `scripts/enrich-tools.ts` warns if the tree is truncated (`[tools] ${repo.full_name} tree was truncated; detection may be incomplete`).