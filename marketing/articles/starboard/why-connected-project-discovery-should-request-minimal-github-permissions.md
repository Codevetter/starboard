...
slug: "why-connected-project-discovery-should-request-minimal-github-permissions"
target_query: "connected project discovery github permissions"
search_intent: "Informational/Evaluative"
meta_title: "Why connected-project discovery should request minimal GitHub permissions"
meta_description: "Learn why developer tools integrating with GitHub projects should request minimal OAuth permissions and how Starboard implements this principle for secure discovery."
---

## Outline
1. Introduction: The balancing act between utility and security
2. The Trust Gap in Developer Tools
3. Understanding GitHub OAuth Scopes and Risks
4. The Principle of Least Privilege in Discovery
5. How Starboard Connects to Public Projects
6. Concrete Examples: Scope Abuse vs. Minimal Scopes
7. Practical Implications for Development Teams
8. Practical Next Action
9. Internal-Link Suggestions
10. Source Notes (Internal Review Only)
...
- **Project-Aware Discovery: Beyond Trending Lists**: Link to this when discussing the transition from generic public discovery to connected-project context.
- **Evidence-Backed Tool Detection**: Link from the section on analyzing manifests and SBOMs to explain how Starboard derives tool recommendations without needing deep, proprietary code access.
- **Understanding Starboard's Architecture**: Link for technical users who want to understand how Starboard manages metadata and embeddings without broad GitHub permissions.

## Source Notes (Internal Review Only)

- **PRODUCT.md**: Confirms that Starboard's "initial project-connection boundary is public GitHub repositories using the existing minimal OAuth permission." It explicitly states that "Private-repository access is an open product and permission decision" and mandates to "Ask for the least GitHub access necessary."
- **PRODUCT.md**: Verifies that "No new production dependencies or broader GitHub OAuth scope are required for the initial connected-project workflow."
- **PROJECT_STATUS.md**: Notes that connected projects and public preview workflows support public GitHub repositories, and that authenticated users can connect them "by URL or an on-demand public-repository picker without a broader OAuth scope."
- **Limitation Context**: The draft accurately reflects that private repository access is not currently supported or requested, avoiding any false claims about private codebase analysis. The article relies purely on current architectural facts rather than invented metrics or customer testimonials.
