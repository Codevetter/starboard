...
describe('maintainer digest', () => {
  ...
  it('groups new additions and suggested actions', () => {
    ...
    expect(
      digest.groups.find((group) => group.id === 'newly_starred')?.items[0]?.starboardUrl
    ).toBe('/explore/owner/repo');
    expect(
      digest.groups.find((group) => group.id === 'suggested_actions')?.items[0]?.actionLabel
    ).toBe('Assign collection');
  });
