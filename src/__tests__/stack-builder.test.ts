...
describe('stack builder', () => {
  ...
  it('classifies starred repos into product stack roles', () => {
    ...
    expect(report.roles.find((role) => role.id === 'framework')?.selected?.fullName).toBe(
      'vercel/next.js'
    );
    expect(report.roles.find((role) => role.id === 'database')?.selected?.fullName).toBe(
      'drizzle-team/drizzle-orm'
    );
    expect(report.roles.find((role) => role.id === 'testing')?.selected?.fullName).toBe(
  ...
  it('detects close alternatives as role conflicts', () => {
    ...
    expect(report.roles.find((role) => role.id === 'framework')?.conflicts[0]).toContain(
      'Choose one primary'
    );
  });
