...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.icon !== undefined) {
    if (body.icon !== null && (typeof body.icon !== 'string' || body.icon.length > 50)) {
      return NextResponse.json(
        { error: 'icon must be null or a string (max 50 chars)' },
        { status: 400 }
      );
    }
    updates.push('icon = ?');
  ...
  if (body.slug !== undefined) {
    if (
      body.slug !== null &&
      (typeof body.slug !== 'string' || !/^[a-z0-9-]{1,100}$/.test(body.slug))
    ) {
      return NextResponse.json(
        { error: 'slug must be null or a lowercase alphanumeric-dash string (max 100 chars)' },
        { status: 400 }
      );
    }
    updates.push('slug = ?');
    args.push(body.slug as string | null);
  }
  if (body.description !== undefined) {
    if (
      ...
      return NextResponse.json(
        { error: 'description must be null or a string (max 500 chars)' },
        { status: 400 }
      );
    }
    updates.push('description = ?');
    args.push(body.description as string | null);
  }
